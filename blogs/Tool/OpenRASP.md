...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000209.png)

这里表明了它的Main-class就是cn.pku.edu.rasp.Agent:

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000247.png)
...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000306.png)

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000325.png)

这里的ENGINE-JAR就是rasp-engine.jar，也就是源代码中的engine模块。这里通过配置文件中的数值通过反射的方式实例化相应的主流程类：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000342.png)

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000402.png)
...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000439.png)

遍历CheckParameter的Type，将其中的元素添加进枚举映射checkers中

Type的枚举类型定义了不同类型的攻击类型所对应的检测方式：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000458.png)
...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000533.png)

这里在实例化了CustomClassTransformer实现的transformer后，调用了自己写的retransform方法。在这个方法中对Instrumentation已加载的所有类进行遍历，将其进行类的重新转换：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000549.png)
...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000607.png)

inst.addTransformer的功能是在类加载时做拦截，对输入的类的字节码进行修改，也就是具体的检测流程插入都在这一部分。addAnnotationHook则完成了加入hook点的工作

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000626.png)
...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000644.png)
...
![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000701.png)

这里直接调用了每个具体hook类的hookMethod方法来执行具体的逻辑。值得注意的是，这里最终返回的也是一个byte数组，具体流程与ASM并无两样。跟进hookMethod：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000718.png)

这里首先生成需要插入到代码中的字节码，然后调用自己写的insertBefore来讲字节码插入到hook点的前面。（这里就是决定是插在hook方法的最顶部，还是return前的最后一行，这决定了调用顺序）。

下面简单看一下插入的字节码是如何生成的：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000734.png)

就是插入一段代码，这段代码将反射实例化当前hook类，调用MethodName所指定的方法，并将paramString所指定的参数传入该方法中。

然后看上面插入的方法，即getBuffer方法的具体逻辑：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000753.png)

将收集到的信息放入一个名为params的HashMap中，然后调用checkBody方法:

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000810.png)

再调用HookHandler.doCheck方法：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000825.png)

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000841.png)

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000857.png)

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000918.png)

这就是检测逻辑，完成：检测计时、获取检测结果以及根据检测结果判断是否要进行拦截。

根据check函数，看一下如何获取的检测结果：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000937.png)

这里的checkers是在hook点管理模块初始化时设置的枚举类映射，所以调用的是XssChecker.check方法：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705000953.png)

其继承树为：XssChecker->ConfigurableChecker->AttackChecker->AbstractChecker，所以最终调用的是AbstractChecker#check方法：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705001008.png)

这里调用的就是XssChecker.checkParam方法：

![img](https://raw.githubusercontent.com/Arielwyy/image-bed/master/img/20220705001024.png)
